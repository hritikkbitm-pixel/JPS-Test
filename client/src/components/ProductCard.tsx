...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        <div className={`bg-white border border-gray-100 rounded-lg product-card flex flex-col relative group overflow-hidden ${!isAvailable ? 'opacity-75 grayscale' : ''}`}>
            <div className="h-48 p-6 flex items-center justify-center relative">
                <img src={product.image} className="max-h-full max-w-full object-contain transition duration-300 group-hover:scale-110" alt={product.name} />
            ...
            <div className="p-4 flex flex-col flex-grow border-t border-gray-50">
                ...
                <h3 className="font-bold text-gray-800 text-sm leading-snug mb-2 line-clamp-2 min-h-[2.5em] group-hover:text-brand-red transition">{product.name}</h3>
