...
export default function PCBuilder() {
    ...
    const availableParts = allParts.filter(p => p.category === currentStep);
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row font-sans">
            {/* LEFT PANEL: Build Summary (Sticky) */}
            <div className="w-full md:w-1/3 lg:w-1/4 bg-white border-r border-gray-200 p-6 flex flex-col h-screen sticky top-0 overflow-y-auto z-10 shadow-lg">
                            ...
                            <div key={step.id} className={`p-3 rounded border transition ${currentStep === step.id ? 'border-brand-red bg-red-50 ring-1 ring-brand-red' : 'border-gray-200 hover:border-gray-300 bg-white'}`}>
                                ...
                                {part ? (
                                    <div className="flex items-center gap-3 mt-2">
                                        <img src={(part as any).image} alt={(part as any).name} className="w-10 h-10 object-contain bg-white rounded border p-1" />
                                        <div className="flex-grow min-w-0">
            ...
            <div className="flex-grow p-8 overflow-y-auto h-screen">
                <div className="max-w-5xl mx-auto">
                    <h2 className="text-3xl font-black text-gray-800 mb-2 uppercase tracking-tight">Select {STEPS.find(s => s.id === currentStep)?.label}</h2>
                    <p className="text-gray-500 mb-8">Choose a component for your build. Incompatible parts are automatically disabled.</p>
                            ...
                            return (
                                <div key={part.id} className={`bg-white rounded-lg border p-4 flex flex-col transition group relative ${isSelected ? 'border-brand-red ring-2 ring-brand-red' : 'border-gray-200 hover:shadow-md'} ${!isCompatible ? 'opacity-50 grayscale' : ''}`}>
                                    ...
                                    <div className="h-40 flex items-center justify-center mb-4 bg-gray-50 rounded p-2">
                                        <img src={part.image} alt={part.name} className="max-h-full max-w-full object-contain mix-blend-multiply" />
                                    </div>
